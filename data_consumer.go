...
func main() {
	consumerTopic := flag.String("consumer", "medical", "Consumer")
	flag.Parse()
	...
	AttrsMap := map[string]string{"Topic": "medical"}
	...
	decryptABETimeArray := []time.Duration{}
	...
	defer func() {
		...
		fmt.Println("Avg Percentile", percentile90, " of ", nTimesFP, "messages = ", p90AvgFP)    		
    		...
    		fmt.Println("== ABE Decrypt ===")
    		nTimesABE := len(decryptABETimeArray)
		ABETimeSum := time.Duration(0)
		
		minTimeABE := decryptABETimeArray[0]
		minTimeABEMemAddr := &minTimeABE
		
		maxTimeABE := decryptABETimeArray[0]
		maxTimeABEMemAddr := &maxTimeABE
		
		for i := 0; i < nTimesABE; i++ {
			ABETimeSum += (decryptABETimeArray[i])
			
			*minTimeABEMemAddr = minTime(minTimeABE, decryptABETimeArray[i])
			*maxTimeABEMemAddr = maxTime(maxTimeABE, decryptABETimeArray[i])
    		}
		...
		nb90TimeABE := int(math.Floor(percentile90*float64(nTimesABE)))    		
    		...
    		for i := 0; i < nb99TimeFP; i++ {
			p99ABETimeSum += (decryptABETimeArray[i])
    		}
    		
    		for i := 0; i < nb95TimeFP; i++ {
			p95ABETimeSum += (decryptABETimeArray[i])
    		}
    		
    		for i := 0; i < nb90TimeFP; i++ {
			p90ABETimeSum += (decryptABETimeArray[i])
    		}
	...
	go func() {
		for {
			...
			case msg := <-partitionConsumer.Messages():
				...
				decoded, _ := b64.StdEncoding.DecodeString(message.MessageBase64)

				//=====================================================================
				//ABE Decrypt - Time START
				...
				decryptABETimeArray = append(decryptABETimeArray, elapsedEncryptABE)
				fmt.Println("ABE Decryption took: %s", elapsedEncryptABE)
				//=====================================================================


				if string(pt) == "startTimeMessage" {
				
					*startMessageTimeAddr = time.Now()
					continue
				
				} else if string(pt) == "endTimeMessage" {				
